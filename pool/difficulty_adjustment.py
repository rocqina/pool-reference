...
def get_new_difficulty(
    ...
    if len(recent_partials) == 0:
        log.info("num of recent_partials is 0")
    ...
    if any(difficulty != current_difficulty for timestamp, difficulty in recent_partials):
        log.info("difficulty != current_difficulty")
        return current_difficulty
    ...
    if current_time - last_timestamp > 3 * 3600:
        log.info(f"current_time - last_timestamp is {current_time - last_timestamp} > 3 * 3600")
        return max(min_difficulty, current_difficulty // 5)

    if current_time - last_timestamp > 3600:
        log.info(f"current_time - last_timestamp is {current_time - last_timestamp} > 3600")
    ...
    if len(recent_partials) < number_of_partials_target and time_taken < time_target:
        log.info(f"len(recent_partials):{len(recent_partials)} number_of_partials_target:{number_of_partials_target}"
    ...
    log.info(f"current_difficulty:{current_difficulty} new_difficulty:{new_difficulty}"
             f" time_taken: {time_taken} time_target:{time_target}")
